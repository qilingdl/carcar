...
#define GPIO13 WIFI_IOT_IO_NAME_GPIO_13
#define GPIO14 WIFI_IOT_IO_NAME_GPIO_14
...
void get_tcrt5000_value (void) {
    ...
    GpioGetInputVal(GPIO13, &id_status);//获取GPIO13引脚的输入电平值
    ...
    GpioGetInputVal(GPIO14, &id_status);//获取GPIO14引脚的输入电平值
...
void TCRT(void)
{
    ...
    if (osThreadNew((osThreadFunc_t)TCRTTask, NULL, &attr) == NULL) {
        printf(" Failed to create RobotCarTestTask!\n");
    }
}
